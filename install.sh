...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="ray"; }
echo 'ray() { "$HOME/core-token/packages/core/bin/run" "$@" --token="ray"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/Ray-ARK/ray_chain_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/Ray-ARK/ray_chain_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@ray"
rm -rf "$HOME/.config/@RayChain"
rm -rf "$HOME/.config/ray-core"
